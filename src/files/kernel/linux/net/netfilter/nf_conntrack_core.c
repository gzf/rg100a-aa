...
#if defined(CONFIG_MIPS_BRCM) && defined(CONFIG_NETFILTER_BRCM_DYNAHELPER)
void (*dynahelper_ref)(struct nf_conntrack_helper *h) = NULL;
EXPORT_SYMBOL_GPL(dynahelper_ref);
void (*dynahelper_unref)(struct nf_conntrack_helper *h) = NULL;
...
#endif
#if defined(CONFIG_NETFILTER_BRCM_DYNAHELPER)
	if (nf_ct_is_confirmed(ct) && help && help->helper && dynahelper_unref)
		dynahelper_unref(help->helper);
#endif
#else
...
#if defined(CONFIG_MIPS_BRCM)
static int regardless_drop(void)
{
...
#if defined(CONFIG_MIPS_BRCM)
	INIT_LIST_HEAD(&conntrack->expectations);
	INIT_LIST_HEAD(&conntrack->derived_connections);
#endif
...
#if defined(CONFIG_MIPS_BRCM)
	/* bugfix for lost connections */
	list_add_tail(&conntrack->safe_list, &safe_list);
#endif
...
#if defined(CONFIG_MIPS_BRCM)
		list_add(&conntrack->derived_list,
			 &exp->master->derived_connections);
		if (exp->flags & NF_CT_EXPECT_DERIVED_TIMEOUT) {
			conntrack->derived_timeout = exp->derived_timeout;
		}
#endif
		if (exp->helper)
	...
	if (atomic_read(&nf_conntrack_count) != 0) {
		...
		if (jiffies >= end) {
			printk("waiting for %d conntrack to be cleaned, "
			       "tried %d times\n",
			       atomic_read(&nf_conntrack_count), try_counter);
			end += HZ;
	}
		try_counter++;
#endif
