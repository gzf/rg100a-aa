...
static int __init bcm6338_hw_init(void)
{
...
#define bcm63xx_specific_hw_init() bcm6338_hw_init()
...
static int __init bcm6348_hw_init(void)
{
    ...
    if( BpGetGPIOverlays(&GPIOOverlays) == BP_SUCCESS ) {
        ...
        if (GPIOOverlays & BP_OVERLAY_MII2) {
            if (GPIOOverlays & BP_OVERLAY_UTOPIA) {
                printk ("*************** ERROR ***************\n");
        ...
        if (GPIOOverlays & BP_OVERLAY_PCI) {
            ...
            GPIO->GPIOMode |= GROUP2_PCI;

            mpi_init();
            if (GPIOOverlays & BP_OVERLAY_CB) {
                GPIO->GPIOMode |= GROUP1_MII_PCCARD;
                mpi_DetectPcCard();
            }
            else {
                /*
...
#define bcm63xx_specific_hw_init() bcm6348_hw_init()
...
#define bcm63xx_specific_hw_init() bcm6358_hw_init()
...
#if defined(CONFIG_BCM_GPON) || defined(CONFIG_BCM_GPON_MODULE)
static void reset_gpon_fpga(void)
{
    int retryCnt = 10;
    unsigned int confDone;

#define GPON_FPGA_CONFIG_SHIFT  26
#define GPON_FPGA_CONFIG_MASK   (1 << GPON_FPGA_CONFIG_SHIFT)

#define GPON_FPGA_DONE_SHIFT    1
#define GPON_FPGA_DONE_MASK     (1 << GPON_FPGA_DONE_SHIFT)

#define GPON_FPGA_RESET_SHIFT   9
#define GPON_FPGA_RESET_MASK    (1 << GPON_FPGA_RESET_SHIFT)

#if defined(GPON_FPGA_PROGRAM)
    /* re-program FPGA */
    printk("Programming GPON FPGA...");

    GPIO->GPIODir |= GPON_FPGA_CONFIG_MASK;

    GPIO->GPIOio &= ~GPON_FPGA_CONFIG_MASK;
    ...
    do
    {
        confDone = (GPIO->GPIOio_high & GPON_FPGA_DONE_MASK) >> GPON_FPGA_DONE_SHIFT;

        printk("GPON FPGA State (%d): 0x%X (0x%08X, 0x%08X / 0x%08X)\n",
               retryCnt, confDone, (uint)GPIO->GPIODir_high, (uint)GPIO->GPIOio_high,
               GPON_FPGA_DONE_MASK);
    ...
    GPIO->GPIODir |= GPON_FPGA_RESET_MASK;

    GPIO->GPIOio &= ~GPON_FPGA_RESET_MASK;
...
void __init allocDspModBuffers(void)
{
#if (defined(CONFIG_BCM96358) || defined(CONFIG_BCM96368)) && defined(CONFIG_BCM_ENDPOINT_MODULE)
